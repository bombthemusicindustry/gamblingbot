    ...
    async def dice(self, ctx, bet, side: int):
        ...
        if random.randint(1, 6) == side:
            ...
            await ctx.send(embed=await macro.send(
                f"You rolled a ``{side}``.\nYou won ``{bet*2}`` {self.currency}.\nYour balance is now ``{bet*3+current}`` {self.currency}.", footer=ctx.message.author, icon=ctx.message.author.avatar_url))
            ...
            await ctx.send(embed=await macro.send(
                f"You rolled a {false_side}.\nYou lost ``{bet}`` {self.currency}.\nYour balance is now ``{current-bet}`` {self.currency}.",footer=ctx.message.author,
    ...
    async def slots(self, ctx, bet):
        ...
        if slot1 == slot2 and slot2 == slot3 and slot3 != 'seven':
            ...
            await msg.edit(embed=await macro.send(
                f"|:{slot1}:|:{slot2}:|:{slot3}:|\nGood! You won ``{bet*2}`` {self.currency}!\nYour balance is now ``{bet*2+current}`` {self.currency}",
            ...
            await msg.edit(embed=await macro.send(
                f"|:{slot1}:|:{slot2}:|:{slot3}:|\nJACKPOT!! You won ``{bet*5}`` {self.currency}!\nYour balance is now ``{bet*5+current}`` {self.currency}",
                footer=ctx.message.author,
                icon=ctx.message.author.avatar_url
            ...
            await msg.edit(embed=await macro.send(
                f"|:{slot1}:|:{slot2}:|:{slot3}:|\nGreat! You won ``{bet*3}`` {self.currency}!\nYour balance is now ``{bet*3+current}`` {self.currency}",
                footer=ctx.message.author,
                icon=ctx.message.author.avatar_url
            ))
            ...
            await self.client.update(bal=current - bet, _id=ctx.message.author.id)
            await msg.edit(embed=await macro.send(
                f"|:{slot1}:|:{slot2}:|:{slot3}:|\nOh no... You lost ``{bet}`` {self.currency}!\nYour balance is now ``{current-bet}`` {self.currency}",
                footer=ctx.message.author,
                icon=ctx.message.author.avatar_url
            ))
